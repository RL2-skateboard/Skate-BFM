## Skate-BFM vendored Isaac runtime

This directory is Skate-BFM's self-contained copy of the BFM-Zero
HumanoidVerse runtime. It contains the training environment implementation,
Hydra configuration, simulator backends, MotionLib, and robot XML, USD, and
mesh assets required by `../train_skate_bfm.py`. Runtime imports and asset
resolution do not depend on `model/bfm-zero-source/`.

The large Isaac Sim/Isaac Lab binary environment is reproducibly installed
from the checked-in lockfile and remains local:
...
This creates `.venv/` with Python 3.10, Isaac Sim 4.5.0, and Isaac Lab 2.0.2.
The local virtual environment is ignored by Git. LAFAN and Skate motion data
remain under `train/dataset/`; checkpoints and generated models remain under
`model/`.

The content below is the upstream BFM-Zero runtime documentation retained for
provenance.
